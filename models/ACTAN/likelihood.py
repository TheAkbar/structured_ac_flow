...
class AutoReg(object):
    ...
    def build(self):
        with tf.variable_scope(self.name, reuse=tf.AUTO_REUSE):
            ...
            self.rnn_out.add(tfk.layers.Dense(self.hps.n_components * 3))
...
def mixture_sample(params_dim, base_distribution='gaussian'):
    '''
    Args:
        params_dim: [B,n*3]
    Return:
...
def mixture_mean(params_dim, base_distribution='gaussian'):
    logits, means, lsigmas = tf.split(params_dim, 3, axis=1)
