...
from .reorder import preprocess, postprocess
...
class Flow(object):
    ...
    def build(self):
        with tf.variable_scope('acflow', reuse=tf.AUTO_REUSE):
            self.trans = Transform('trans', self.hps)
            self.prior = Prior('prior', self.hps)
    ...
    def cond_forward(self, x, y, b, m):
        x_u, x_o = preprocess(x, b, m)
        c = tf.concat([tf.one_hot(y, self.hps.n_classes), x_o], axis=1)
