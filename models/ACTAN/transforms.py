...
class Linear(BaseTransform):
    ...
    def build(self):
        ...
        with tf.variable_scope(self.name, reuse=tf.AUTO_REUSE):
            ...
            self.wnn.add(tfk.layers.Dense(d*d, name=f'l{i+1}', kernel_initializer=tf.zeros_initializer()))
            self.bnn.add(tfk.layers.Dense(d, name=f'l{i+1}', kernel_initializer=tf.zeros_initializer()))

    def get_perm_matrix(self, query):
        order = tf.contrib.framework.argsort(query, direction='DESCENDING', stable=True)
        return tf.batch_gather(tf.matrix_diag(query), order)

    def get_matrix_parts(self, c, b, m):
        d = self.hps.dimension
        B = tf.shape(c)[0]
        h = tf.concat([c, b, m], axis=1)
        wc = self.wnn(h)
        wc = tf.reshape(wc, [B,d,d])
        weight = wc + self.w
        # reorder
        t = self.get_perm_matrix(m * (1-b))
        weight = tf.matmul(tf.matmul(t, weight), tf.transpose(t, perm=[0,2,1]))
        U = tf.matrix_band_part(weight, 0, -1)
        L = tf.eye(d) + (weight - U)
    ...
    def get_params(self, c, b, m):
        ...
        bc = self.bnn(h)
        bias = bc + self.b
        U, L = self.get_matrix_parts(c, b, m)

        query = m * (1-b)
        t = self.get_perm_matrix(query)
        weight = tf.matmul(L, U, name='A')
        ...
        U += diag
        ldet = tf.reduce_sum(tf.log(tf.abs(tf.matrix_diag_part(U))), -1)
        
        return weight, bias, ldet

    def forward(self, x, c, b, m):
        weight, bias, ldet = self.get_params(c, b, m)
...
class LULinear(BaseTransform):
    ...
    def forward(self, x, c, b, m):
        ...
        #     tf.check_numerics(z, 'linear->z'),
        #     tf.check_numerics(ldet, 'linear->ldet')
        # ]):
        #     z = tf.identity(z)
        #     ldet = tf.identity(ldet)
...
class Affine(BaseTransform):
    ...
    def forward(self, x, c, b, m):
        ...
        # # debug
        # with tf.control_dependencies([
        #     tf.check_numerics(z, 'affine->z'),
        #     tf.check_numerics(ldet, 'affine->ldet')
        # ]):
        #     z = tf.identity(z)
        #     ldet = tf.identity(ldet)
